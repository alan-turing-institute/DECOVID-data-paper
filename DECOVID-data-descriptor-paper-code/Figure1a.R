...
# install.packages(
#    c("DBI","dplyr", "Hmisc", "purrr","lubridate", "aweek", "tidyr",
#      data.table", "stringr", "ggplot2", "epiR", "extrafont", "forcats",
#      "reshape2","sqldf"))
...
person_level_table <- paste0(
  ...
  paste0(paste0(
    "'",
    omop_covid_all %>% pull(visit_occurrence_id),
    "'",
    collapse = ","
  )),
  ")
...
omop_person_table <-
  ...
  mutate(
    ...
    ethnicity_group = case_when(
      grepl(race_concept_name,
            pattern = "Asian or Asian British:") ~ "Asian",
      grepl(race_concept_name,
            pattern = "Black or African or Caribbean or Black British:") ~
        "Black",
      grepl(race_concept_name,
            pattern = "Mixed multiple ethnic groups:") ~ "Mixed",
      grepl(race_concept_name,
            pattern = "White:") ~ "White",
      grepl(race_concept_name,
            pattern = "Other ethnic group:") ~ "Other",
      race_concept_name == "Ethnicity not stated" |
...
Total <-
  Ethnicity[, list(sum = sum(n)), by = c("covid", "hospital_site")]
...
Ethnicity$Percentage_label <-
  (format(round((
    Ethnicity$n / Ethnicity$sum
  ) * 100, 1), nsmall = 1))

# Recode Ethnicity groups
Ethnicity$Ethnicity <-
  ifelse(
    grepl("Asian", Ethnicity$ethnicity_group),
    "Asian or Asian British",
    ifelse(
      grepl("Black", Ethnicity$ethnicity_group),
      "Black or African or Caribbean or Black British",
      ifelse(
        grepl("Mixed", Ethnicity$ethnicity_group),
        "Mixed or Multiple ethnic groups",
        ifelse(
          grepl("Unknown", Ethnicity$ethnicity_group),
          "Not stated/Unknown",
...
Ethnicity$covid <-
  ifelse(Ethnicity$covid == "No", "Non-COVID-19", "COVID-19")

# Create the Ethnicity component of  Figure 1a of the paper
EthnicityComponent.FigA <-
  ggplot(Ethnicity, aes(fill = covid, x = Ethnicity, y = Percentage)) +
...
EthnicityComponent.FigA <-
  EthnicityComponent.FigA + geom_text(
    aes(label = Percentage_label),
    position = position_dodge(width = 0.9),
    hjust = -0.2,
    size = 8
  ) +   labs(x = "", fill = "COVID-19 Status")

# Save plot
ggsave(
  png_file("Ethnicity_Figure1a_DECOVIDPaper"),
  height = 6,
  width = 14.2,
  plot = EthnicityComponent.FigA
)

# Write to csv to save data
write.csv(
  Eth_Personlevel,
  csv_file("Eth_Personlevel_July2022"),
  na = "",
  row.names = F
)
...
# age-sex pyramid
PyramidData$n <-
  ifelse(PyramidData$gender_concept_name == "Female",
         (-1) * (PyramidData$n),
         PyramidData$n)

# Rename columns
colnames(PyramidData) <-
  c("AgeGroup", "COVIDStatus", "Hospital", "Sex", "Count")
...
PyramidData$Count_max <-
  ifelse(PyramidData$COVIDStatus == "Yes", 2500, 21000)
PyramidData$Count_min <-
  ifelse(PyramidData$COVIDStatus == "Yes",-2500,-21000)
PyramidData[, x_min := floor(min(Count_min)), by = COVIDStatus]
PyramidData[, x_max := ceiling(max(Count_max)) , by = COVIDStatus]

PyramidData$COVIDStatus <-
  factor(PyramidData$COVIDStatus, levels = c("Yes", "No"))
...
# Save plot
ggsave(
  png_file("AgeSex_Pyramid_Figure1a_DECOVIDPaper"),
  height = 5,
  width = 11.4,
  plot = Age_Sex_Pyramid
)

# Write to csv to save data
write.csv(
  Age_Sex_Split,
  csv_file("Age_Sex_Split_July2022"),
  na = "",
  row.names = F
)
  ...
  "SELECT
   ...
   FROM
       (SELECT
        ...
        FROM measurement
        WHERE (measurement_concept_id=37310255) AND
              (value_as_concept_id=37310282)) a
     INNER JOIN
       (SELECT
          fact_id_1,
     ...
     WHERE (
       DATEDIFF(day, CAST(visit_start_datetime as date), specimen_date) <= 14
       AND
       (specimen_date <= CAST(visit_end_datetime as date)))
     OR (DATEDIFF(day, CAST(visit_start_datetime as date), specimen_date) <= 14
         AND
         (CAST(visit_end_datetime as date) IS NULL))"
)

# Confirmed/suspected COVID-19 Query
covid_obs_all_query_plot <-
  paste(
    "SELECT
       a.visit_occurrence_id,
       person_id,
       'CONDITION' AS case_type,
     ...
     FROM
       (SELECT *
        ...
        WHERE
          condition_concept_id IN 
            (45590872, 703441, 37310287, 45604597, 37311060, 703440, 37310282,
             439676, 45585955, 37311061, 45756093, 45756094, 320651, 37310268)
       ) a
       INNER JOIN
       (SELECT
          visit_start_datetime,
       ...
       WHERE
         (DATEDIFF(day,
                   CAST(visit_start_datetime as date),
                   condition_start_date) <= 14
          AND (condition_start_date <= CAST(visit_end_datetime as date)))
         OR (DATEDIFF(day,
                      CAST(visit_start_datetime as date),
                      condition_start_date) <= 14
             AND (CAST(visit_end_datetime as date) IS NULL))"
  )


# Run PCR Only Queries
omop_covid_pcr_plot <-
  dbGetQueryBothTrusts(db, covid_pcr_query_plot) %>%
  distinct()
...
# because we want PCR positive case type to take precedence over the CONDITION
# case type when remove duplicates
...
omop_covid_all_plot_sort <-
  omop_covid_all_plot_sort[
    !duplicated(omop_covid_all_plot_sort$visit_occurrence_id),
    ]

# Create field for hospital Trust
omop_covid_all_plot_sort$hospital_site <-
  ifelse(omop_covid_all_plot_sort$schema == "uhb", "UHB", "UCLH")

# Generate a column for week, as the plot will be by week
omop_covid_all_plot_sort$week <-
  cut((omop_covid_all_plot_sort$covid_date), "week")
...
# We cannot show values mop_c sid_weekly$n_eq <- ife10 for the plot
omop_covid_weekly$n_eq <-
  ifelse(omop_covid_weekly$n <= 10, 10, omop_covid_weekly$n)
...
WeekTSPlot <-
  ggplot(omop_covid_weekly,
         aes(x = week_graph, y = n_eq, fill = hospital_site)) +
...
WeekTSPlot <-
  ...
  labs(fill = "Hospital Trust") +
  guides(fill = guide_legend(nrow = 1, ncol = 2))

# Save graph
ggsave(
  png_file("COVIDCasesTimeSeries_DECOVIDPaper"),
  height = 6.5,
  width = 12,
  plot = WeekTSPlot
)
...
write.csv(
  omop_covid_weekly,
  csv_file("omop_covid_weekly_DECOVIDPaper"),
  na = "",
  row.names = F
)
...
concept_ancestor_table <-
  dbGetQueryBothTrusts(db, concept_ancestor_q,
                       schemas = "dbo")
...
concept_relationship_table_standard_conditions <-
...
concept_relationship_replaces_cond <- conditions_standard %>%
  ...
  inner_join(
    select(
      concept_relationship_replaces_cond,
      concept_id_2,
      concept_id_1
    ),
    by = c("condition_concept_id" = "concept_id_1"),
...
conditions_standard <-
  rbind(conditions_standard, concept_relationship_replaces_cond)

# Check missing
missing_standard_cond <-
  setdiff(omop_cond_all_v2$condition_concept_id,
          conditions_standard$condition_concept_id)

Nonstandard <- concept %>%
  filter(concept_id %in% missing_standard_cond) %>%
  select(concept_id, concept_name)

omop_cond_all_v2$hospital_site <-
  ifelse(omop_cond_all_v2$schema == "uhb", "UHB", "UCLH")
...
HDs <- concept_ancestor_table %>%
  ...
  left_join(conditions_standard,
            by = c("descendant_concept_id" = "concept_id_standard"))
...
# Just to check the diagnoses included in phenotype being created here
ChronicKidney_names <-
  data.frame(unique(ChronicKidney$concept_name_specific))
...
Comoborbidities = omop_person_table %>%
  mutate(
    HeartDisease = ifelse(person_id %in% (HDs$person_id), 1, 0),
...
Summary_Comoborbidities <-
  sqldf(
    ...
    GROUP BY hospital_site, covid"
  )

Summary_Comoborbidities$covid <-
  factor(Summary_Comoborbidities$covid, levels = c("Yes", "No"))
levels(Summary_Comoborbidities$covid) <-
  c("COVID-19", "Non-COVID-19")

# Create long data set to create figure
Summary_Comoborbidities_long <-
  melt(
    Summary_Comoborbidities,
    id.vars = c("hospital_site", "covid", "N"),
    variable.name = "condition"
  )

# Calculate percentage that have a comorbidity by hospital and COVID status
Summary_Comoborbidities_long$Percent  <-
  round((
    Summary_Comoborbidities_long$value / Summary_Comoborbidities_long$N
  ) * 100,
  1)
...
Summary_Comoborbidities_long$Percent_label <-
  format(round((
    Summary_Comoborbidities_long$value / Summary_Comoborbidities_long$N
  ) * 100,
  1
...
Summary_Comoborbidities_long$condition_formal_name <-
  ifelse(
    Summary_Comoborbidities_long$condition == "HeartDisease",
    "Heart Disease",
    ifelse(
      Summary_Comoborbidities_long$condition == "Diabetes",
      "Diabetes Mellitus (any type)",
      ifelse(
        Summary_Comoborbidities_long$condition == "Resp",
        "Chronic Respiratory Disease",
...
Comoborbidities_Fig1a <-
  ggplot(Summary_Comoborbidities_long,
         aes(
           x = fct_reorder(condition_formal_name, Percent),
           y = (Percent),
           fill = covid
         )) +
  geom_bar(position = "dodge", stat = "identity") +
...
Comoborbidities_Fig1a <-
  Comoborbidities_Fig1a + geom_text(
...
ggsave(
  png_file("Comorbidities_Figure1a_DECOVIDPaper"),
  height = 6,
  width = 14.5,
  plot = Comoborbidities_Fig1a
)

# Save data for reference for later
write.csv(
  Summary_Comoborbidities_long,
  csv_file("Summary_Comoborbidities_long"),
  na = "",
  row.names = F
)

# All images were brought into a PowerPoint slide and formatted and exported
# as an image file for the final DECOVID Data Descriptor manuscript
