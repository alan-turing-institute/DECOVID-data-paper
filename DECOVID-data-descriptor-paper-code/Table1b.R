...
#install,packages("DBI", "dplyr", "Hmisc", "purrr")
...
measurement_query <- paste0("SELECT * FROM
                        (SELECT visit_occurrence_id, 
                                visit_occurrence_id %10 as hospital_site, 
                                day,
                                length_of_stay, 
                        ...
                        (SELECT visit_occurrence_id, 
                            ...
                            paste0(paste0("'", measurement %>% filter(!(category %in% c("blood_gas","blood_pressure"))) %>% pull(measurement_concept_id), "'", collapse=",")), ") AND visit_occurrence_id IS NOT NULL) b
...
measurement_query_blood <- paste0("SELECT * FROM
                            (SELECT visit_occurrence_id, 
                                    visit_occurrence_id %10 as hospital_site, 
                                    day, 
                                    length_of_stay, 
...
bloodgas_query <- paste0("SELECT * FROM
                                  ...
                                  ELSE CEILING((DATE_PART('day', measurement_datetime::timestamp - visit_detail_start_datetime::timestamp))  +
                                      ...
                                      (SELECT visit_occurrence_id, 
                                              ...
                                              measurement_id, 
                                              measurement_concept_id FROM omop_03082021.measurement
                                      WHERE measurement_concept_id IN (3010421, 3013290, 3027315) AND visit_occurrence_id IS NOT NULL) b
                                      INNER JOIN  
...
for (i in 1:length(unique(omop_measure_all$measurement_concept_name))) {
     ...
     covid_hospital = omop_measure_l2_l3_any_visit_type %>%
...
#write to csv
write.csv(Medians_L2_L3, "Medians_L2_L3.csv" , na="", row.names=FALSE)
...
rbind(omop_count_all_l2_l3_any %>%
        ...
        write.csv("omop_measure_mean_missing_l2_l3_anyvisittype.csv", row.names=FALSE, na="")

###############################################
#Now, do the exact same, but for Level 1 care #
###############################################
...
write.csv(MediansL1_dataframe, "MediansL1_dataframe.csv" , na="")
...
rbind(omop_count_all_l1_any_visit_type %>%
        ...
        mutate("Summary"=rep("% missing"))) %>% arrange(measurement_concept_name,category) %>%
        write.csv("omop_measure_l1_mean_missing_any_visit_type.csv", row.names=FALSE, na="")
