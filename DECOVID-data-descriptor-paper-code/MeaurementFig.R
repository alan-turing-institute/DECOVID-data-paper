    ...
    "SELECT
     ...
     (SELECT
        ...
        (DATEDIFF(minute,
                  ...
                  visit_end_datetime) / 1440) AS patient_days
     ...
     LEFT JOIN
     (SELECT
        concept_id as admitting_source_concept_id,
...
omop_visit_clean_all_org <- omop_visit_all_org %>%
  ...
  mutate(
    ethnicity_group = case_when(
      grepl(race_concept_name, pattern = "Asian or Asian British:") ~
        "Asian",
      grepl(race_concept_name,
            pattern = "Black or African or Caribbean or Black British:") ~
        "Black",
      grepl(race_concept_name, pattern = "Mixed multiple ethnic groups:") ~
        "Mixed",
      grepl(race_concept_name, pattern = "White:") ~
        "White",
      grepl(race_concept_name, pattern = "Other ethnic group:") ~
        "Other",
  ...
  mutate(
    l2_l3 = ifelse(
      visit_occurrence_id %in% (
        omop_care %>% filter(care_site_id %in% c(1, 2, 3, 15, 20, 21, 22)) %>%
    ...
    Inpatient = ifelse(
      visit_occurrence_id %in% (
        omop_care %>% filter(care_site_id %in% c(7, 8, 17)) %>%
          pull(visit_occurrence_id)
      ),
      "Yes",
      "No"
    ...
    ED = ifelse(
      visit_occurrence_id %in% (
        omop_care %>% filter(care_site_id %in% c(9, 10, 11, 12, 13, 18, 19)) %>%
          pull(visit_occurrence_id)
      ),
      "Yes",
      "No"
    ...
    Other = ifelse(
      visit_occurrence_id %in% (
        omop_care %>% filter(care_site_id %in% c(4, 5, 6, 14, 16)) %>%
          pull(visit_occurrence_id)
      ),
      "Yes",
      "No"
...
paste("Number of visits:",
      nrow(omop_visit_all),
      "-",
      sum(duplicated(omop_visit_all$visit_occurrence_id)),
      "duplicates")
...
CreateTableOne(
  ...
  strata = c("COVID diagnosis", "Hospital"),
  addOverall = T,
  test = F,
  includeNA = T
) %>% print(
  showAllLevels = T,
  nonnormal = c("Year of birth", "Length of stay (days)"),
  noSpaces = T
)


# Care site query with the # of days in each visit detail record from
# visit_detail table. This is the only we can calculate the number patient days
# in each level of care
care_site_query_measurments <- paste0(
  "SELECT
     *,
...
omop_care <-
  dbGetQueryBothTrusts(db, care_site_query_measurments) %>%
  distinct() %>%
  mutate(
...
for (i in 1:length(measurement_nobp_no_resp$measurement_concept_id)) {
  ...
  meas_q_figs <- paste0(
    ...
    paste0(
      paste0(
        "'",
        measurement_nobp_no_resp[i, ] %>% pull(measurement_concept_id),
        "'",
        collapse = ""
      )
    ),
    " AND visit_occurrence_id IS NOT NULL) a
     LEFT JOIN
  ...
  omop_meas_all <- omop_meas_all %>%
    mutate(
      hospital_site = case_when(schema == "uhb" ~ "UHB",
                                schema == "uclh" ~ "UCLH"),
      covid = if_else(
        as.character(visit_occurrence_id) %in%
  ...
  omop_care$visit_detail_id_char <-
    as.character(omop_care$visit_detail_id)
  omop_meas_all$visit_detail_id_char <-
    as.character(omop_meas_all$visit_detail_id)
  omop_meas_all_care <-
    inner_join(omop_meas_all,
               omop_care,
  ...
  # time
  omop_meas_all_carefilter$measurement_time_re <-
    as.POSIXct(omop_meas_all_carefilter$measurement_datetime,
               format = "%Y-%m-%d %H:%M:%S")
  omop_meas_all_carefilter$measurement_time_re <-
    format(omop_meas_all_carefilter$measurement_time_re, format = "%H:%M:%S")
  omop_meas_all_carefilter$measurement_time_re <-
    as_hms(omop_meas_all_carefilter$measurement_time_re)
  omop_meas_all_carefilter$Level <-
    ifelse(
      omop_meas_all_carefilter$care_site_id %in%
        c("1", "2", "3", "15", "20", "21", "22"),
      "Level 2/3",
      ifelse(
        omop_meas_all_carefilter$care_site_id %in%
  ...
  omop_meas_all_carefilter$Level <-
    as.factor(omop_meas_all_carefilter$Level)
  
  RR_d_UCLH <-
    ggplot(
      omop_meas_all_carefilter %>% filter(Level != "Other" &
                                            hospital_site.x == "UCLH"),
      aes(measurement_time_re)
    ) +
    geom_density() +
    ...
    ggtitle(
      paste(
        "UCLH: ",
        omop_meas_all_carefilter$measurement_concept_name,
        "\n(Concept ID: ",
        omop_meas_all_carefilter$measurement_concept_id,
        ")",
        sep = ""
      )
    ) +
  ...
  ggsave(png_file(paste(
    "MeasurementTimeSamplePlot_UCLH",
    unique(omop_meas_all_carefilter$measurement_concept_id),
    sep = ""
  )), plot = RR_d_UCLH)
...
meas_q_figs <- paste0(
  ...
  paste0(paste0(
    "'",
    measurement_bp %>% pull(measurement_concept_id),
    "'",
    collapse = ","
  )),
  ") AND visit_occurrence_id IS NOT NULL) a
...
omop_meas_all$visit_detail_id_char <-
  as.character(omop_meas_all$visit_detail_id)
omop_meas_all$visit_occurrence_id <-
  as.character(omop_meas_all$visit_occurrence_id)
omop_care$visit_occurrence_id <-
  as.character(omop_care$visit_occurrence_id)
omop_meas_all_care <-
  inner_join(omop_meas_all,
             omop_care,
             by = c("visit_occurrence_id"),
...
omop_meas_all_carefilter$measurement_time_re <-
  as.POSIXct(
    strptime(omop_meas_all_carefilter$measurement_datetime,
             format = "%Y-%m-%d %H:%M:%S")
  )
omop_meas_all_carefilter$measurement_time_re <-
  format(omop_meas_all_carefilter$measurement_time_re, format = "%H:%M:%S")
omop_meas_all_carefilter$measurement_time <-
  as_hms(omop_meas_all_carefilter$measurement_time_re)
omop_meas_all_carefilter$Level <-
  ifelse(
    omop_meas_all_carefilter$care_site_id %in%
    ...
    "Level 2/3 Care",
    ifelse(
      omop_meas_all_carefilter$care_site_id %in% c("7", "8", "17"),
      "Level 1 Care",
      "Other"
    )
  )
...
BLOODPRESSUREDATA <-
  omop_meas_all_carefilter %>% filter(Level != "Other" &
                                        hospital_site.x == "UCLH")
BLOODPRESSUREDATA$measurement_time_re <-
  as_hms(BLOODPRESSUREDATA$measurement_time_re)
BLOODPRESSUREDATA$measurement_time <-
  as.POSIXct(BLOODPRESSUREDATA$measurement_time_re)
...
RR_d_UCLH <- ggplot(BLOODPRESSUREDATA, aes(measurement_time)) +
  ...
  ggtitle(paste(
    ...
    "\n(Concept IDs: ",
    paste0(unique(
      BLOODPRESSUREDATA$measurement_concept_id
    ), collapse = ","),
    ")",
    sep = ""
  )) +
...
ggsave(
  filename = png_file(
    paste("MeasurementTimeSamplePlot_UCLH", "BloodPressure", sep = "")
  ),
  height = 7,
  width = 7,
  plot = RR_d_UCLH
...
ggsave(
  filename = png_file(
    paste0("MeasurementTimeSamplePlot_UHB", "BloodPressure", sep = "")
  ),
  height = 7,
  width = 7,
  plot = RR_d_UHB
...
meas_q_figs <- paste0(
  "SELECT
   ...
   FROM
     (SELECT *
     FROM measurement
     WHERE measurement_concept_id IN ('4313591', '4108138', '4154772')
           AND
           visit_occurrence_id IS NOT NULL) a
     LEFT JOIN
     (SELECT concept_id as measurement_id,
             concept_name as measurement_concept_name,
             concept_class_id as concept_class_id_measurement,
...
omop_meas_all$visit_occurrence_id <-
  as.character(omop_meas_all$visit_occurrence_id)
omop_meas_all_care <-
  inner_join(omop_meas_all, omop_care, by = c("visit_occurrence_id"))
...
omop_meas_all_carefilter$Level <-
  as.factor(omop_meas_all_carefilter$Level)
omop_meas_all_carefilter$time <-
  as.POSIXct(omop_meas_all_carefilter$measurement_time_re)
...
RR_d_UCLH <- ggplot(RR_UCLH , aes(measurement_time_re)) +
  ...
  ggtitle(paste(
    ...
    "Respiratory Rate",
    "\n(Concept ID: ",
    paste0(unique(RR_UCLH$measurement_concept_id), collapse = ","),
    ")",
    sep = ""
  )) +
  xlab("Time of Day") +
...
ggsave(
  filename = png_file(paste(
    "MeasurementTimeSamplePlot_UCLH", "RespRate", sep = ""
  )),
  height = 7,
  width = 7,
  plot = RR_d_UCLH
...
RR_d_UHB <- ggplot(RR_UHB, aes(measurement_time_re)) +
  ...
  ggtitle(paste(
    ...
    "Respiratory Rate",
    "\n(Concept IDs: ",
    paste0(unique(RR_UHB$measurement_concept_id), collapse = ","),
    ")",
    sep = ""
  )) +
  xlab("Time of Day") +
...
ggsave(
  filename = png_file(paste(
    "MeasurementTimeSamplePlot_UHB", "RespRate", sep = ""
  )),
  height = 7,
  width = 7,
  plot = RR_d_UHB
